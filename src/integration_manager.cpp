...
IntegrationResult
IntegrationManager::performIntegration(AppState &state,
...
std::optional<GslFunctionParams>
IntegrationManager::getFunctionParams(AppState &state, LuaManager &luaManager) {
	auto func = luaManager.getFunction(state.selectedFunctionName);
	if (!func.has_value()) {
		state.currentScriptError = "Could not find valid function '" +
...
bool IntegrationManager::performGslIntegration(
	AppState &state, IntegrationResult &result,
	...
	case GslIntegrationMethod::QAGS:
		status = gsl_integration_qags(&F, state.x_min, state.x_max, 1e-7, 1e-7,
									  1000, workspace, &result.result, &abserr);
		break;

	case GslIntegrationMethod::QAGI:
		status = gsl_integration_qagi(&F, 1e-7, 1e-7, 1000, workspace,
									  &result.result, &abserr);
		break;

	case GslIntegrationMethod::QAGIU:
		status = gsl_integration_qagiu(&F, state.x_min, 1e-7, 1e-7, 1000,
									   workspace, &result.result, &abserr);
		break;

	case GslIntegrationMethod::QAGIL:
		status = gsl_integration_qagil(&F, state.x_max, 1e-7, 1e-7, 1000,
									   workspace, &result.result, &abserr);
		break;

	case GslIntegrationMethod::QAG:
		status = gsl_integration_qag(&F, state.x_min, state.x_max, 1e-7, 1e-7,
									 1000, state.qag_key, workspace,
									 &result.result, &abserr);
		break;
	}
...
bool IntegrationManager::handleIntegrationStatus(int status, AppState &state,
												 IntegrationResult &result,
												 double abserr) {
...
bool IntegrationManager::generatePlotPoints(AppState &state,
											IntegrationResult &result,
											const GslFunctionParams &params) {
...
void IntegrationManager::initializePlotVectors(IntegrationResult &result,
											   int num_points) {
...
bool IntegrationManager::calculateExtendedPoints(
	double plot_x_min, double plot_x_max, IntegrationResult &result,
	const GslFunctionParams &params) {
	for (size_t i = 0; i < result.extended_x_points.size(); i++) {
		try {
			double x = plot_x_min + (plot_x_max - plot_x_min) * i /
										(result.extended_x_points.size() - 1.0);
			result.extended_x_points[i] = x;
			result.extended_y_points[i] =
				gslFunctionWrapper(x, const_cast<GslFunctionParams *>(&params));

			if (std::isnan(result.extended_y_points[i]) ||
				std::isinf(result.extended_y_points[i])) {
				return false;
			}
		} catch (...) {
			return false;
...
bool IntegrationManager::calculateIntegrationPoints(
	const AppState &state, IntegrationResult &result,
	const GslFunctionParams &params) {
	switch (state.selectedIntegrationMethod) {
	case GslIntegrationMethod::QAGS:
	case GslIntegrationMethod::QAG:
		return calculatePoints(state.x_min, state.x_max, result.x_points,
							   result.y_points, params);
	...
	case GslIntegrationMethod::QAGIU:
		return calculatePoints(state.x_min, state.x_min + 10.0, result.x_points,
							   result.y_points, params);

	case GslIntegrationMethod::QAGIL:
		return calculatePoints(state.x_max - 10.0, state.x_max, result.x_points,
							   result.y_points, params);
	}
	return true;
}

bool IntegrationManager::calculatePoints(double start_x, double end_x,
	...
	for (size_t i = 0; i < x_points.size(); i++) {
		try {
			double x =
				start_x + (end_x - start_x) * i / (x_points.size() - 1.0);
			x_points[i] = x;
			y_points[i] =
				gslFunctionWrapper(x, const_cast<GslFunctionParams *>(&params));
		} catch (...) {
...
void IntegrationManager::calculatePlotRange(const AppState &state,
											double &plot_x_min,
											double &plot_x_max) {
	switch (state.selectedIntegrationMethod) {
	case GslIntegrationMethod::QAGS:
...
double IntegrationManager::gslFunctionWrapper(double x, void *params) {
	...
	try {
		...
		gsl_integration_error =
			std::string("Error in function evaluation: ") + e.what();
		return 0.0;
	}
}

void IntegrationManager::handlePlotGenerationError(AppState &state,
												   IntegrationResult &result) {
...
void IntegrationManager::setCurrentScriptError(AppState &state,
											   IntegrationResult &result,
